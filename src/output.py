...
def gap_between_bands(output_png:str, mb_manager: MemberBandManager, sorted_bands: List[Band]) -> None:
    gaps = []
...
def output_schedule(output_csv: str, mb_manager: MemberBandManager, sorted_bands: List[Band]) -> None:
    with open(output_csv, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["時刻"] + list(sorted_bands[0].info.keys()))
        for i, schedule in enumerate(mb_manager.schedules):
            writer.writerow([schedule.name] + ["" if pd.isna(item) else item for item in sorted_bands[i].info.values])

def output_members_schedule(output_csv: str, mb_manager: MemberBandManager, sorted_bands: List[int]) -> None:
    with open(output_csv, 'w', newline='') as f:
        writer = csv.writer(f)
